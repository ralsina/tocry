...
module MCPTestHelpers
  ...
  def assert_success_response(response : Hash(String, JSON::Any))
    response["success"].as_bool.should be_true
    response.should_not contain "error"
  end

  def assert_error_response(response : Hash(String, JSON::Any))
    response["success"].as_bool.should be_false
    response.keys.should contain("error")
    response["error"].as_s.should_not be_empty
  end
