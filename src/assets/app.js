  ...
  try {
    ...
    boardSelector.addEventListener("change", async (event) => {
      ...
      if (selectedValue === "__NEW_BOARD__") {
        ...
        history.pushState(
          { board: previousBoardSelection },
          "",
          `/b/${previousBoardSelection}`
        );
  ...
  if (laneName !== null && laneName.trim() !== "") {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const totalNotes = currentLanes.reduce(
          (total, lane) => total + lane.notes.length,
          0
        );
        if (currentLanes.length === 1 && totalNotes === 1) {
          showNotification(
...
async function handleDeleteLaneRequest(laneName) {
  if (
    await showConfirmation(
      `Are you sure you want to delete the lane "${laneName}"?`,
      "Delete Lane"
    )
  ) {
    try {
...
async function handleUpdateLaneNameRequest(laneToUpdate, newName) {
  ...
  try {
    ...
    const currentPosition = currentLanes.findIndex(
...
async function handleUpdateNoteTitleRequest(noteToUpdate, newTitle) {
  ...
  try {
    ...
    const response = await updateNote(currentBoardName, noteToUpdate.id, {
...
async function handleToggleNoteRequest(noteToUpdate, isExpanded) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const lane = currentLanes.find((l) =>
...
async function handleEditNoteSubmit(event) {
  ...
  try {
    ...
    const response = await updateNote(currentBoardName, noteId, {
      note: notePayload,
      lane_name: null,
      position: null,
    });
...
async function handleLaneDrop(event) {
  ...
  const draggedLaneIndex = currentLanes.findIndex(
    (lane) => lane.name === draggedLaneName
  );
  const targetLaneIndex = currentLanes.findIndex(
  ...
  const draggedLaneElement = document.querySelector(
    `.lane[data-lane-name="${draggedLaneName}"]`
  );
  const targetLaneElement = document.querySelector(
    `.lane[data-lane-name="${targetLaneName}"]`
  );
  ...
  const tempLanes = currentLanes.filter(
    (lane) => lane.name !== draggedLaneName
  );
...
async function handleNoteDrop(event) {
  ...
  if (targetLaneName !== originalLane) {
    // Move between lanes
    const targetLaneObject = currentLanes.find(
      (lane) => lane.name === targetLaneName
    );
    if (!targetLaneObject) {
    ...
    originalLaneObject.notes = originalLaneObject.notes.filter(
      (note) => note.id !== noteId
    );
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  (async () => {
    // 1. Determine the initial board from URL. The global `currentBoardName` is 'default' initially.
    ...
    await initializeLanes(currentBoardName);
  })();
