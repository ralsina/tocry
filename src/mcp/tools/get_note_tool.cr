  ...
  def initialize
    super(
      ...
      parameters: {
        ...
        "required" => JSON::Any.new(["note_id"].map { |s| JSON::Any.new(s) })
      },
  ...
  def invoke_with_user(params : Hash(String, JSON::Any), user_id : String) : Hash(String, JSON::Any)
    ...
    begin
...

      {
        ...
        "tags" => JSON::Any.new(note_result.tags.map { |tag| JSON::Any.new(tag) }),
        "priority" => JSON::Any.new(note_result.priority.to_s),
        "start_date" => note_result.start_date ? JSON::Any.new(note_result.start_date.not_nil!) : JSON::Any.new(nil),
        "end_date" => note_result.end_date ? JSON::Any.new(note_result.end_date.not_nil!) : JSON::Any.new(nil),
        "public" => JSON::Any.new(note_result.public),
        "attachments" => JSON::Any.new(note_result.attachments.map { |att| JSON::Any.new(att) }),
