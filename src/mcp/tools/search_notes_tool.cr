...
class SearchNotesTool < ModelContextProtocol::Server::Tool
  def initialize
    super(
      ...
      parameters: {
        ...
        "required" => JSON::Any.new(["query"].map { |s| JSON::Any.new(s) })
      },
  ...
  def invoke_with_user(params : Hash(String, JSON::Any), user_id : String) : Hash(String, JSON::Any)
    ...
    begin
      ...
      boards_to_search.each do |board|
        board.lanes.each do |lane|
          lane.notes.each do |note|
            ...
            matches_query = note.title.downcase.includes?(query) ||
                           ...
                           note.tags.any? { |tag| tag.downcase.includes?(query) }
            ...
            if matches_query && matches_priority
              results << {
                ...
                "board_name" => JSON::Any.new(board.name),
                "board_id" => JSON::Any.new(board.sepia_id),
                ...
                "start_date" => note.start_date ? JSON::Any.new(note.start_date.not_nil!) : JSON::Any.new(nil),
                "end_date" => note.end_date ? JSON::Any.new(note.end_date.not_nil!) : JSON::Any.new(nil),
                "public" => JSON::Any.new(note.public),
                "attachments" => JSON::Any.new(note.attachments.map { |att| JSON::Any.new(att) })
              }
            end
          end
        end
      ...
      {
        "results" => JSON::Any.new(results.map { |r| JSON::Any.new(r) }),
