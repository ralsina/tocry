  ...
  def initialize
    super(
      ...
      parameters: {
        ...
        "required" => JSON::Any.new(["board_name", "lane_name", "title"].map { |s| JSON::Any.new(s) })
      },
  ...
  def invoke_with_user(params : Hash(String, JSON::Any), user_id : String) : Hash(String, JSON::Any)
    ...
    begin
...

      {
        ...
        "start_date" => new_note.start_date ? JSON::Any.new(new_note.start_date.not_nil!) : JSON::Any.new(nil),
        "end_date" => new_note.end_date ? JSON::Any.new(new_note.end_date.not_nil!) : JSON::Any.new(nil),
