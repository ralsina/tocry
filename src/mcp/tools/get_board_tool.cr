  ...
  def initialize
    super(
      ...
      parameters: {
        ...
        "required" => JSON::Any.new(["board_name"].map { |s| JSON::Any.new(s) })
      },
    ...
    begin
      ...
      lanes_data = board.lanes.map do |lane|
...

        {
          ...
          "notes" => JSON::Any.new(notes_data.map { |n| JSON::Any.new(n) }),
...

      {
        ...
        "lanes" => JSON::Any.new(lanes_data.map { |l| JSON::Any.new(l) }),
