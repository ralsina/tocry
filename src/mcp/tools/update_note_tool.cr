  ...
  def initialize
    super(
      ...
      parameters: {
        ...
        "required" => JSON::Any.new(["note_id", "board_name"].map { |s| JSON::Any.new(s) })
      },
  ...
  def invoke_with_user(params : Hash(String, JSON::Any), user_id : String) : Hash(String, JSON::Any)
    ...
    begin
...

      {
        ...
        "start_date" => current_note.start_date ? JSON::Any.new(current_note.start_date.not_nil!) : JSON::Any.new(nil),
        "end_date" => current_note.end_date ? JSON::Any.new(current_note.end_date.not_nil!) : JSON::Any.new(nil),
